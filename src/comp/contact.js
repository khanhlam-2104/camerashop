...
const Contact = () => {
    ...
    const send = async (e) =>{
        const{Name, email, subject, Messega} = user
        e.preventDefault()
        const option = {
            method: 'POST',
            headers: {
        ...
        const send = await fetch(
            'https://react-ecommerce-contact-52049-default-rtdb.firebaseio.com/Messega.json', option
            )
        if(send){
            alert("Tin Nhắn Của Bạn Đã Được Gửi Đi")
        }
        else{
            alert("Lỗi Gửi Không Thành Công")
        }
    }
  return (
    <>
                ...
                <div className='form'>
                    ...
                    <form method='POST'>
                        ...
                        <div className='box'>
                            ...
                            <div className='input'>
                                <textarea placeholder='Message' value={user.Messega} name='Messega' onChange={data} ></textarea>
                            </div>
                        </div>
